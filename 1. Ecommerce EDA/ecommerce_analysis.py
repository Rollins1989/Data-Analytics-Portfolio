...
def build_analysis(df):
    ...
    by_region["avg_order_value"] = (
        by_region["revenue"] / by_region["orders"]
    ).round(2)
    ...
    discount = (
        df.assign(discount_pct=(df["discount"] * 100).round().astype(int))
        .groupby("discount_pct", as_index=False)
        .agg(avg_revenue=("revenue", "mean"), orders=("order_id", "count"))
    )
...
def save_outputs(results):
    ...
    report = {
        ...
        "customer_summary": results["customer_summary"],
    }
...
def create_charts(results):
    ...
    results["by_category"].plot.bar(
        x="category", y="revenue", legend=False, title="Revenue by Category"
    )
    plt.ylabel("Revenue")
    plt.tight_layout()
    ...
    results["monthly"].plot.line(
        x="month", y="revenue", marker="o", legend=False, title="Monthly Revenue"
    )
    plt.ylabel("Revenue")
    plt.tight_layout()
